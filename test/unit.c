...
void testStringBuffer() {
        ...
        printf("=> Test6: prepare2postgres\n");
        {
                ...
                TRY
                {
                        StringBuffer_prepare2postgres(sb);
                ...
                // Just 99 ?'s
                sb= StringBuffer_new("???????????????????????????????????????????????????????????????????????????????????????????????????");
                assert(StringBuffer_prepare2postgres(sb) == 99);
